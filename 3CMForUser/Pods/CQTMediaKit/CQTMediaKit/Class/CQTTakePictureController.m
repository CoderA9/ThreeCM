//
//  CQTTakePictureViewController.m
//  CQTKeyTime
//
...
#define    kMaxVideonDuration    30
#define    kTagIndicator         111

@implementation CQTTakePictureController
...
- (id)initWithType:(UIImagePickerControllerSourceType)sType {
...
@interface CQTAssetPickerController ()
...
@implementation CQTVideoTitleView
...
-(void)drawRect:(CGRect)rect{
    ...
    UIImage *videoIcon=[UIImage imageNamed:@"pickerVideo_icon.png"];
...
@interface CQTTapAssetView ()

@property(nonatomic,retain)UIImageView *selectView;

@end

@implementation CQTTapAssetView
...
+ (void)initialize
{
    checkedIcon     = [UIImage imageNamed:@"pickerChecked.png"];
    
    checkedNoIcon     = [UIImage imageNamed:@"pickerNotChecked.png"];
...
@interface CQTAssetView ()<CQTTapAssetViewDelegate>

@property (nonatomic, strong) ALAsset *asset;

@property (nonatomic, weak) id<CQTAssetViewDelegate> delegate;

@property (nonatomic, retain) UIImageView *imageView;
@property (nonatomic, retain) CQTVideoTitleView *videoTitle;
@property (nonatomic, retain) CQTTapAssetView *tapAssetView;

@end

@implementation CQTAssetView
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
        ...
        _videoTitle=[[CQTVideoTitleView alloc] initWithFrame:CGRectMake(0, kThumbnailSize.height-20, kThumbnailSize.width, titleHeight)];
        ...
        _tapAssetView=[[CQTTapAssetView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
...
#pragma mark - CQTTapAssetView Delegate
...
@interface CQTAssetViewCell ()<CQTAssetViewDelegate>

@end

@class CQTAssetViewController;

@implementation CQTAssetViewCell
    ...
    if (self.contentView.subviews.count<assets.count) {
        ...
        for (int i=0; i<assets.count; i++) {
            ...
            if (i>((NSInteger)self.contentView.subviews.count-1)) {
                ...
                CQTAssetView *assetView=[[CQTAssetView alloc] initWithFrame:CGRectMake(assetViewX+(kThumbnailSize.width+minimumInteritemSpacing)*i, minimumLineSpacing-1, kThumbnailSize.width, kThumbnailSize.height)];
                [assetView bind:assets[i] selectionFilter:selectionFilter isSeleced:[(CQTAssetViewController*)_delegate containAsset:assets[i]]];
                ...
                ((CQTAssetView*)self.contentView.subviews[i]).frame=CGRectMake(assetViewX+(kThumbnailSize.width+minimumInteritemSpacing)*(i), minimumLineSpacing-1, kThumbnailSize.width, kThumbnailSize.height);
                
                [(CQTAssetView*)self.contentView.subviews[i] bind:assets[i] selectionFilter:selectionFilter isSeleced:[(CQTAssetViewController*)_delegate containAsset:assets[i]]];
            }
        ...
        for (int i=self.contentView.subviews.count; i>0; i--) {
            if (i>assets.count) {
                [((CQTAssetView*)self.contentView.subviews[i-1]) removeFromSuperview];
            }
            else{
                ((CQTAssetView*)self.contentView.subviews[i-1]).frame=CGRectMake(assetViewX+(kThumbnailSize.width+minimumInteritemSpacing)*(i-1), minimumLineSpacing-1, kThumbnailSize.width, kThumbnailSize.height);
                [(CQTAssetView*)self.contentView.subviews[i-1] bind:assets[i-1] selectionFilter:selectionFilter isSeleced:[(CQTAssetViewController*)_delegate containAsset:assets[i - 1]]];
            }
        }
    }
}

#pragma mark - CQTAssetView Delegate
...
@interface CQTAssetViewController ()<CQTAssetViewCellDelegate>{
    int columns;
...
@implementation CQTAssetViewController
...
//    [self.navigationItem.rightBarButtonItem setBackgroundImage:[UIImage imageNamed:@"complete.png"] forState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
...
- (BOOL)containAsset:(ALAsset *)asset {
    ...
    NSString *terminalStr = asset.defaultRepresentation.url.absoluteString;
    
    for (ALAsset * asset in  self.indexPathsForSelectedItems) {
        
        if ([asset.defaultRepresentation.url.absoluteString isEqualToString:terminalStr]) {
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CQTAssetPickerController *picker = (CQTAssetPickerController *)self.navigationController;
    
    CQTAssetViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell==nil) {
        cell=[[CQTAssetViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
    }
    cell.delegate=self;
...
#pragma mark - CQTAssetViewCell Delegate

- (BOOL)shouldSelectAsset:(ALAsset *)asset
{
    CQTAssetPickerController *vc = (CQTAssetPickerController *)self.navigationController;
...
- (void)didDeselectAsset:(ALAsset *)asset
{
    if ([_indexPathsForSelectedItems containsObject:asset]) {
        ...
        [self removeAssetWithUrl:asset.defaultRepresentation.url.absoluteString];
    }
...
- (void)removeAssetWithUrl:(NSString *)assetUrl {
    
    for (ALAsset * asset in _indexPathsForSelectedItems) {
        
        if ([asset.defaultRepresentation.url.absoluteString isEqualToString:assetUrl]) {
...
@interface CQTAssetGroupViewCell ()

@property (nonatomic, strong) ALAssetsGroup *assetsGroup;

@end

@implementation CQTAssetGroupViewCell
...
@interface CQTAssetGroupViewController()
...
@implementation CQTAssetGroupViewController
...
- (void)setupGroup
{
    if (!self.assetsLibrary)
        self.assetsLibrary = [CQTTakePictureController defaultAssetsLibrary];
...
- (void)showNotAllowed
{
    ...
    UIImageView *padlock    = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"pickerResourceLocked.png"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CQTAssetGroupViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil)
    {
        cell = [[CQTAssetGroupViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:CellIdentifier];
    }
...
{
    CQTAssetViewController *vc = [[CQTAssetViewController alloc] init];
    vc.assetsGroup = [self.groups objectAtIndex:indexPath.row];
    
    CQTAssetPickerController *nv = (CQTAssetPickerController *)self.navigationController;
    if (nv && [nv isKindOfClass:[CQTAssetPickerController class]]) {
        
        vc.indexPathsForSelectedItems = nv.indexPathsForSelectedItems;
    }
...
@implementation CQTAssetPickerController



- (id)init
{
    CQTAssetGroupViewController *groupViewController = [[CQTAssetGroupViewController alloc] init];
