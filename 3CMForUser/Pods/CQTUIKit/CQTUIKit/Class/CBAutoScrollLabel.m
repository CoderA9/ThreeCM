...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        self.tFont = [UIFont systemFontOfSize:14.0];
    }
    return self;
}
...
//    NSMutableSet *labelSet = [[NSMutableSet alloc] initWithCapacity:kLabelCount];
    self.labels = [[NSMutableArray alloc] initWithCapacity:kLabelCount];
...
//    self.labels = [NSArray arrayWithArray:labelSet.allObjects];
...
- (void)setTFont:(UIFont *)tFont {
    if (self.mainLabel.font == tFont)
        return;
    
    EACH_LABEL(font, tFont)
