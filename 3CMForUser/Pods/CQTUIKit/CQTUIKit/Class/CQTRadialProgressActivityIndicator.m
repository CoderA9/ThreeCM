...
#define CQT_DEGREES_TO_RADIANS(x) (x)/180.0*M_PI
#define CQT_RADIANS_TO_DEGREES(x) (x)/M_PI*180.0
...
/*-----------------------------------------------------------------*/
@interface CQTRadialProgressActivityIndicator()
...
@end
@implementation CQTRadialProgressActivityIndicator
...
- (void)_commonInit
{
    ...
    self.imageLayer.transform = CATransform3DMakeRotation(CQT_DEGREES_TO_RADIANS(180),0,0,1);

    //init arc draw layer
    CAShapeLayer *shapeLayer = [[CAShapeLayer alloc] init];
    shapeLayer.frame = self.bounds;
    shapeLayer.fillColor = nil;
    shapeLayer.strokeColor = HEX_RGB(0xE94708).CGColor;
...
//    CQTDebugLog(@"center.x = %f, center.y = %f",center.x,center.y);
    
    self.shapeLayer.path = [UIBezierPath bezierPathWithArcCenter:center radius:self.bounds.size.width/2 - 1 startAngle:-M_PI_2 endAngle:-M_PI_2 + 2 * M_PI clockwise:YES	].CGPath;
}
...
- (void)setProgress:(double)progress
{
    ...
    if (progress >= 0 && progress <=1.0) {
        ...
        animationImage.fromValue = [NSNumber numberWithFloat:CQT_DEGREES_TO_RADIANS(180-180*prevProgress)];
        animationImage.toValue = [NSNumber numberWithFloat:CQT_DEGREES_TO_RADIANS(180-180*progress)];
        animationImage.duration = 0.15;
        animationImage.removedOnCompletion = NO;
        animationImage.fillMode = kCAFillModeForwards;
        [CATransaction setDisableActions:YES];
        self.imageLayer.transform = CATransform3DMakeRotation(CQT_DEGREES_TO_RADIANS(180-180*progress), 0, 0, 1);
