...
#import "CQTMBProgressHUD.h"

@interface CQTMBProgressHUD ()
...
@implementation CQTMBProgressHUD
...
- (void)updateLabelText:(NSString *)newText {
    if (labelText != newText) {
        ...
        CQT_RELEASE(labelText);
        labelText = CQT_RETAIN(newText);
    }
}
...
- (void)updateDetailsLabelText:(NSString *)newText {
    if (detailsLabelText != newText) {
        ...
        CQT_RELEASE(detailsLabelText);
        detailsLabelText = CQT_RETAIN(newText);
    }
}
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeDeterminate) {
        ...
        self.indicator = CQT_AUTORELEASE([[MBRoundProgressView alloc] init]);
    }
    else if (mode == MBProgressHUDModeCustomView && self.customView != nil){
        self.indicator = self.customView;
    } else {
		self.indicator = CQT_AUTORELEASE([[UIActivityIndicatorView alloc]
...
+ (CQTMBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	CQTMBProgressHUD *hud = [[CQTMBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
	return CQT_AUTORELEASE(hud);
}

+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	UIView *viewToRemove = nil;
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[CQTMBProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		CQTMBProgressHUD *HUD = (CQTMBProgressHUD *)viewToRemove;
...
- (void)dealloc {
    ...
    A9_ObjectReleaseSafely(indicator);
    A9_ObjectReleaseSafely(label);
    A9_ObjectReleaseSafely(detailsLabel);
    A9_ObjectReleaseSafely(labelText);
    A9_ObjectReleaseSafely(detailsLabelText);
    A9_ObjectReleaseSafely(graceTimer);
    A9_ObjectReleaseSafely(minShowTimer);
    A9_ObjectReleaseSafely(showStarted);
    A9_ObjectReleaseSafely(customView);
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        /*
        NSArray* carList = [self.labelText componentsSeparatedByString:@"\n"];
        CGSize dims = CGSizeZero;
        for (NSString* cstr in carList) {
             CGSize sSizeMsg = [cstr sizeWithFont:self.labelFont];
              dims.height += sSizeMsg.height;
        }
        label.numberOfLines = [carList count];
        */
...
- (void)showWhileExecuting:(SEL)method onTarget:(id)target withObject:(id)object animated:(BOOL)animated {
    ...
    targetForExecution = CQT_RETAIN(target);
    objectForExecution = CQT_RETAIN(object);
...
- (void)cleanUp {
    ...
    A9_ObjectReleaseSafely(targetForExecution);
    A9_ObjectReleaseSafely(objectForExecution);
